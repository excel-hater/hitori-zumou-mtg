...
4. 「編集」でその日の記録を Markdown で自由に書き換えて保存できます。コピーやダウンロード（`asakai-YYYY-MM-DD.md`）もここから
5. 📅 でカレンダーを開くと、過去の日の朝会の内容を見たり、メモを付けたり、編集したりできます
...
- 「使い方」で会話の例と、編集・カレンダー・記録の保存についての説明を見られます
...
- 外部通信ゼロ（CDN・Web フォント・解析なし）。合計 約38KB（上限 200KB）
...
| `calendar.js` | カレンダーの日付計算（純粋関数） |
| `export.js` | セッションを Markdown に変換。編集した内容があればそちらを優先 |
